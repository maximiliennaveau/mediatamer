...
def find_embedded_sub(infile: Path) -> Optional[str]:
    ...
    try:
        ...
        # Prefer English
        english_subs = [l for l in candidates if re.search(
            r'\b(eng|english)\b', l, re.I)]
        if english_subs:
            match = re.search(r'track\s*(\d+)', english_subs[0], re.I)
            if match:
                return match.group(1)
        # Then French
        french_subs = [l for l in candidates if re.search(
            r'\b(fra|fre|french|vost)\b', l, re.I)]
        if french_subs:
...
def build_handbrake_cmd(infile: Path, outfile: Path, srtfile: Optional[Path], embedded_sub: Optional[str]) -> list[str]:
...
def main():
    ...
    exts = {e.lower() if e.startswith(
        '.') else f".{e.lower()}" for e in (args.exts or VIDEO_EXTS)}

    # Check HandBrakeCLI
    if not subprocess.run(['which', 'HandBrakeCLI'], capture_output=True).returncode == 0:

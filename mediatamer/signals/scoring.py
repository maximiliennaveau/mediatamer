...
def score_episode_match(
    ...
    if has_chapters:
        if 4 <= chapter_count <= 8:
            score += 20.0
            reasons.append(
                f"Chapter count ({chapter_count}) consistent with single episode"
            )
        elif chapter_count > 12 and technical_signals.is_multi_episode:
            score += 40.0
            reasons.append(
                f"High chapter count ({chapter_count}) suggesting multi-episode file"
            )
    ...
    if context_hints:
        ...
        if is_likely_episode is True:
            if int(ep.get("season_number", -1)) == 0:
                ...
                and int(ep.get("season_number", -1)) == target_season
            ):
                score += 30.0
                reasons.append(
                    f"Rewarding Season {target_season} for likely main episode file"
                )
            ...
            if int(ep.get("season_number", -1)) == 0:
                ...
                score -= 50.0
                reasons.append(
                    f"Penalizing Season {target_season} for likely bonus file"
                )
        ...
        if last_episode_matched is not None:
            if ep_num == last_episode_matched + 1:
                score += 60.0
                reasons.append(
                    f"Perfect relative sequence (last E{last_episode_matched} -> E{ep_num})"
                )
            elif ep_num > last_episode_matched:
                score += 30.0
                reasons.append(
                    f"Forward episode sequence (E{ep_num} > last E{last_episode_matched})"
                )
            elif ep_num <= last_episode_matched:
                score -= 60.0  # Strict penalty for out of order
                reasons.append(
                    f"Penalizing out-of-order sequence (E{ep_num} <= last E{last_episode_matched})"
                )
        ...
        if disc_info:
            ...
            if global_idx is not None:
                ...
                if ep_num == expected_ep_abs:
                    ...
                    reasons.append(
                        f"Global index match (t{global_idx:02d} -> E{ep_num})"
                    )
                    ...
                    if dvd_number is None or dvd_number <= 1:
                        score -= 100.0
                        reasons.append(
                            f"Global index mismatch on DVD1 (expected E{expected_ep_abs}, got E{ep_num})"
                        )
                    else:
                        ...
                        if last_episode_matched is None:
                            score -= 20.0
                            reasons.append(
                                f"Gentle penalty for global mismatch on DVD{dvd_number} (first file)"
                            )
                        else:
    ...
    def score_text(text, weight, source_name):
        nonlocal score
        if not text or not ep.get("name"):
            return
        title = ep["name"].lower()
        t_lower = text.lower()
        if title in t_lower:
    ...
    if context_hints and "llm_scores" in context_hints:
        llm_scores = context_hints["llm_scores"]
        ep_num = ep.get("episode_number")
        llm_score = llm_scores.get(ep_num, 0.0)
        if llm_score > 0:
            weight = 150.0
            added_score = llm_score * weight
            score += added_score
            reasons.append(
                f"LLM discrimination score: {llm_score:.2f} (+{added_score:.1f})"
            )

...
class EpisodeMatcher:
    ...
    def find_metadata(self) -> None:
        ...
        if self.show_name:
            ...
            if self._detected_lang and self._detected_lang != "en":
                print(
                    f"  [LANG] Detected subtitle language: {self._detected_lang} → fetching TMDB in {self._tmdb_locale}"
                )
    ...
    def _infer_context(self):
        ...
        meta = infer_context_from_path(self.metadata)
        ...
        if show_name:
            ...
            self.show_name = None
            print(
                f"Could not infer show name and season from {self.file_path.parent.name}."
            )
    ...
    def _fetch_tmdb_episodes(self):
        ...
        normalized_name, episodes = fetch_tmdb_episodes(
            self.show_name, self.season_number, self.tmdb_api_key, locale
        )
    ...
    def _match_file(self) -> List[Dict[str, Any]]:
        ...
        for ep in self.tmdb_episodes:
            ...
            candidates.append(
                {"episode": ep, "score": res["score"], "reasons": res["reasons"]}
            )

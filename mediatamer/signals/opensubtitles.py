...
class OpenSubtitleSignals:
    ...
    def _fetch_metadata(self, moviehash: str) -> Optional[Dict[str, Any]]:
        if not self.api_key:
            raise ValueError("OpenSubtitles API key is missing from the configuration (opensubtitles-api-key).")
            
        url = f"https://api.opensubtitles.com/api/v1/subtitles?moviehash={moviehash}"
        headers = {
            "Api-Key": self.api_key,
            "User-Agent": "MediaTamer v1.0"
        }
    ...
    def extract(self) -> None:
        try:
            ...
            if not moviehash:
                print("Could not compute OpenSubtitles hash (file too small or unavailable).")

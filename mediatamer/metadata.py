...
def find_parent_show_and_season(file_path: Path, input_root: Path) -> Tuple[str, Optional[int]]:
    ...
    if not show_dir:
        ...
        while current != input_root and current != current.parent:
            dir_name = current.name
            if not extract_season_only(dir_name) and dir_name.lower() not in ['tv-shows', 'tv_shows', 'shows', 'series', 'movies', 'films', 'videos', 'media', 'unsorted_videos', 'unsorted']:
...
def lookup_episode_info(show_name: str, season: int, episode: int, api_key: str = None, language: str = 'en-US'):
    ...
    if not api_key:
        print("Warning: No TMDB API key provided. Get one at https://www.themoviedb.org/settings/api and use --tmdb-api-key")
        return None
...
def extract_metadata(path: Path, input_root: Path = None, api_key: str = None, language: str = 'en-US') -> Dict[str, Any]:
    ...
    if input_root:
        ...
        if (extract_season_only(root_name) is not None or
                'special' in root_name or 'extra' in root_name or 'bonus' in root_name):
        ...
        title_is_useful = (embedded_title and
                           len(embedded_title) > 2 and
                           not embedded_title.lower() in ['stereo', 'mono', 'left', 'right', 'audio', 'video'] and
                           not re.search(r'\b(stereo|mono|audio|video|track|channel)\b', embedded_title, re.I))
...
def write_csv_summary(outdir: Path, rows: List[Dict[str, Any]]):
    ...
    keys = [
        'filename', 'filepath', 'show_name', 'season', 'episode_start', 'episode_end', 'is_special', 'episode_title', 'duration', 'size', 'bit_rate',

...
def cache_subtitle(video_path: Path, cache_dir: Path, force: bool = False):
    ...
    credits_text = extract_credits_text(
        video_path, opening_duration=180.0, closing_duration=180.0
    )
...
def main():
    ...
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=Path(os.getenv("SUBTITLE_CACHE_DIR")),
        help="Directory to store cached subtitles (default env: SUBTITLE_CACHE_DIR)",
    )
    parser.add_argument(
        "--force", action="store_true", help="Force re-extraction even if cache exists"
    )
    parser.add_argument(
        "--extensions",
        nargs="*",

...
class TestPrefixDetection(unittest.TestCase):
    ...
    def test_multi_prefix_detection_real_fs(self, mock_matcher_cls, mock_tech):
        ...
        def technical_side_effect(path):
            name = Path(path).name
            if name.startswith('A'): return {'duration': 45*60}
            if name.startswith('B'): return {'duration': 43*60}
        ...
        self.assertTrue(results["A1.mkv"].is_likely_episode, "A1 should be likely episode")
        self.assertTrue(results["A2.mkv"].is_likely_episode, "A2 should be likely episode")
        self.assertTrue(results["B1.mkv"].is_likely_episode, "B1 should be likely episode")
        self.assertTrue(results["B2.mkv"].is_likely_episode, "B2 should be likely episode")
